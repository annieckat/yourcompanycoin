...
def update_team(client,mysql_cn,user_id,team_to_update_id,team_to_update_monthly_transfer):
    ...
    try:
        ...
        mycursor.execute(f'''
        update teams set monthly_transfer = {int(team_to_update_monthly_transfer)}, updated_at = @current_datetime
        where team_id = '{team_to_update_id}';
        ''')
    except Exception as e:
        logging.exception(f'{user_id} update team Error: '+str(e))
        mysql_cn.rollback()
        mycursor.close()
        return('error')
    mysql_cn.commit()
