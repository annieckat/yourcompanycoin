...
def delete_user(client,mysql_cn,user_id,user_to_update_id):
    ...
    try:
        ...
        mycursor.execute(f'''
        update users set is_deleted = 1,updated_at = @current_datetime where user_id = '{user_to_update_id}';
        ''')
    except Exception as e:
        logging.exception(f'{user_id} deletion user Error: '+str(e))
        mysql_cn.rollback()
        mycursor.close()
        return('error')
    mysql_cn.commit()
